...
export const showSolarSystem = (id) => {
  ...
  // Mercúrio
  const mercury = createPlanet(3, mercuryTexture, 70);

  // Vênus
  const venus = createPlanet(4.6, venusTexture, 120);

  // Terra
  const earth = createPlanet(6, earthTexture, 220);

  // Marte
  const mars = createPlanet(7.8, marsTexture, 300);

  // Júpiter
  const jupiter = createPlanet(20, jupiterTexture, 410);

  // Saturno (contém anéis)
  const saturn = createPlanet(15, saturnTexture, 500, {
    innerRadius: 10,
    outerRadius: 20,
  ...
  // Urano
  const uranus = createPlanet(12, uranusTexture, 700);

  // Netuno
  const neptune = createPlanet(10, neptuneTexture, 800);
