...
export const showSolarSystem = (id) => {
  ...
  const sunMat = new THREE.MeshBasicMaterial({
    map: textureLoad.load(sunTexture.src),
  });
  ...
  // Mercúrio
  const mercury = createPlanet(3, mercuryTexture.src, 70);

  // Vênus
  const venus = createPlanet(4.6, venusTexture.src, 120);

  // Terra
  const earth = createPlanet(6, earthTexture.src, 220);

  // Marte
  const mars = createPlanet(7.8, marsTexture.src, 300);

  // Júpiter
  const jupiter = createPlanet(20, jupiterTexture.src, 410);

  // Saturno (contém anéis)
  const saturn = createPlanet(15, saturnTexture.src, 500, {
    innerRadius: 10,
    outerRadius: 20,
  ...
  // Urano
  const uranus = createPlanet(12, uranusTexture.src, 700);

  // Netuno
  const neptune = createPlanet(10, neptuneTexture.src, 800);
